...
if (userStr) {
    ...
    // Verify it's a real Firebase user, not a guest
    if (currentUser.uid.startsWith('guest_')) {
        ...
        // Hide auth modal if user is properly logged in
...
async function signInWithGoogle() {
    ...
    try {
        ...
        if (error.code === 'auth/unauthorized-domain') {
            alert("⚠️ Google Sign-In is not available on localhost.\n\n" +
                  "To use this app, you need to:\n" +
                  "1. Deploy to GitHub Pages, Firebase Hosting, or another hosting service\n" +
                  "2. Add your domain to Firebase Console → Authentication → Settings → Authorized domains\n\n" +
                  "For localhost testing, add 'localhost' to authorized domains in Firebase.");
        } else if (error.code === 'auth/popup-blocked') {
            alert("Popup was blocked. Please allow popups for this site and try again.");
        } else if (error.code === 'auth/cancelled-popup-request' || error.code === 'auth/popup-closed-by-user') {
            // User closed the popup, just show the auth screen again
        } else {
...
function continueAsGuest() {
    alert("⚠️ Guest mode has been disabled.\n\n" +
          "This app requires Firebase authentication to sync and store your data securely.\n\n" +
          "Please sign in with Google to continue.\n\n" +
          "Note: If you're testing locally, you need to:\n" +
        ...
        .then((snapshot) => {
            ...
            if (snapshot.exists()) {
                ...
                if (firebaseData.budgets && Array.isArray(firebaseData.budgets)) {
                    budgets = firebaseData.budgets.map(budget => ({
                        id: budget.id,
                        date_range: budget.date_range,
                        budget: parseFloat(budget.budget) || 0,
                        remaining: parseFloat(budget.remaining) || 0,
                        total_spend: parseFloat(budget.total_spend) || 0,
                        progress: parseFloat(budget.progress) || 0,
                        saved_at: budget.saved_at || new Date().toISOString(),
                ...
                if (firebaseData.expenses && Array.isArray(firebaseData.expenses)) {
                    transactions = firebaseData.expenses.map(expense => ({
                        id: `${expense.id}_${expense.row_index}`,
                ...
                if (firebaseData.settings) {
                    if (firebaseData.settings.currency_code) {
                        currentCurrency = firebaseData.settings.currency_code;
                        console.log('Currency loaded from Firebase:', currentCurrency);
                        // Update all currency displays
                        updateAllCurrencyDisplays();
                    }
                }
                
                console.log(`✓ Synced ${firebaseData.budgets?.length || 0} budgets and ${firebaseData.expenses?.length || 0} expenses from Firebase`);
                
                // Set currentDateRange to the most recent budget (like desktop app does)
                if (budgets.length > 0) {
                    const sortedBudgets = [...budgets].sort((a, b) => new Date(b.saved_at) - new Date(a.saved_at));
                    currentDateRange = sortedBudgets[0].date_range;
                    console.log('Auto-selected date range:', currentDateRange);
                } else {
                    currentDateRange = null;
                    console.log('No budgets found, showing all data');
                }
...
let isFirstSyncCall = true;

function setupRealtimeSync() {
    if (!currentUser || !currentUser.uid || firebaseListener) {
        return; // Already listening or no user
    }
    
    console.log('🔄 Setting up real-time Firebase sync listener...');
    isFirstSyncCall = true;
    
    // Listen for changes to user's data
    const dataRef = database.ref(`users/${currentUser.uid}/data`);
    dataRef.on('value', (snapshot) => {
        // Skip the first call since we just loaded data with .once()
        if (isFirstSyncCall) {
            isFirstSyncCall = false;
            console.log('✓ Real-time listener active, watching for changes...');
            return;
        }
        
        if (snapshot.exists()) {
            const firebaseData = snapshot.val();
            console.log('🔔 Real-time update detected from desktop app!');
            
            // Update budgets in memory
            if (firebaseData.budgets && Array.isArray(firebaseData.budgets)) {
                budgets = firebaseData.budgets.map(budget => ({
                    id: budget.id,
                    date_range: budget.date_range,
            ...
            if (firebaseData.expenses && Array.isArray(firebaseData.expenses)) {
                ...
                console.log('  → Expenses updated:', transactions.length);
            }
            
            // Update currency settings in real-time
            if (firebaseData.settings && firebaseData.settings.currency_code) {
                if (currentCurrency !== firebaseData.settings.currency_code) {
                    currentCurrency = firebaseData.settings.currency_code;
                    console.log('  → Currency updated:', currentCurrency);
                    updateAllCurrencyDisplays();
                }
            }
            ...
            if (typeof updateSevenDayChart === 'function') updateSevenDayChart();
            if (typeof updateBudgetOverview === 'function') updateBudgetOverview();
            if (typeof updateSummaryTable === 'function') updateSummaryTable();
            
            console.log('✅ Web app UI refreshed from desktop app changes!');
        }
    });
    
    firebaseListener = dataRef;
}

function stopRealtimeSync() {
    if (firebaseListener && currentUser) {
        database.ref(`users/${currentUser.uid}/data`).off('value', firebaseListener);
...
function initializeApp() {
    ...
    // Trigger the main DOMContentLoaded initialization
    const event = new Event('appReady');
    document.dispatchEvent(event);
}

// Initialize app - all data from Firebase (no localStorage)
let budgets = [];
let transactions = [];
...
function saveBudgetForDateRange(dateRange, budgetAmount) {
...
function getBudgetIdForDateRange(dateRange) {
    const entry = budgets.find(b => b.date_range === dateRange);
    return entry ? entry.id : null;
}

function isDateInRange(transactionDate, dateRangeString) {
    if (!dateRangeString || dateRangeString === 'All') return true;
    
    try {
        // Parse date range like "Jan 1 - Jan 31" or "Dec 25 - Jan 5"
        ...
        const startMonth = new Date(Date.parse(startParts[0] + ' 1, ' + currentYear)).getMonth();
        const startDay = parseInt(startParts[1]);
        const endMonth = new Date(Date.parse(endParts[0] + ' 1, ' + currentYear)).getMonth();
        ...
        // Handle year wrap-around (e.g., Dec 25 - Jan 5)
        if (endDate < startDate) {
            // Check if transaction is in the next year portion
            if (txDate.getMonth() <= endMonth) {
...
function updateAllCurrencyDisplays() {
    // Refresh all UI components to show new currency
    if (typeof renderTransactionsTable === 'function') renderTransactionsTable();
    if (typeof updateBudgetOverview === 'function') updateBudgetOverview();
    if (typeof updateSummaryTable === 'function') updateSummaryTable();
    if (typeof updateSevenDayChart === 'function') updateSevenDayChart();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize currentDateRange from most recent budget (will be set after Firebase loads)
    if (budgets.length > 0) {
        const sortedBudgets = [...budgets].sort((a, b) => new Date(b.saved_at) - new Date(a.saved_at));
        currentDateRange = sortedBudgets[0].date_range;
        console.log('Page load: currentDateRange set to', currentDateRange);
    }
    ...
    // Google Sign In Button in Profile Modal
    ...
    if (googleSignInBtn) {
        googleSignInBtn.addEventListener('click', async () => {
            if (currentUser) {
                ...
                if (confirmSignOut) {
                    const resetData = confirm('Do you want to reset all local data?\n\n⚠️ This will delete all your expenses and budgets from this device.\n\nYour cloud data will remain safe if you\'ve synced before.');
                    ...
                    if (resetData) {
                        ...
                        // Refresh UI
                        await initializeApp();
                        showNotification('All local data has been cleared successfully.');
                    }
        ...
        if (budgetRanges.length > 0) {
            ...
            budgetRanges.forEach((entry, idx) => {
                ...
                div.addEventListener('click', () => {
                    ...
                    if (mobileDatePickerDisplay) mobileDatePickerDisplay.textContent = entry.date_range;
            ...
            allDiv.addEventListener('click', () => {
                ...
                if (mobileDatePickerDisplay) mobileDatePickerDisplay.textContent = 'All';
                datePickerDropdown.classList.remove('active');
                document.querySelectorAll('.date-range-item').forEach(el => el.classList.remove('active'));
            ...
            if (selected) {
                ...
                if (activeItem) {
                    ...
                    if (mobileDatePickerDisplay) mobileDatePickerDisplay.textContent = selected;
                ...
                if (firstItem) {
                    ...
                    if (mobileDatePickerDisplay) mobileDatePickerDisplay.textContent = budgetRanges[0].date_range;
                }
            }
        } else {
    ...
    // Mobile date picker setup
    const mobileDatePickerBtn = document.getElementById('mobileDatePickerBtn');
    const mobileDatePickerDisplay = document.getElementById('mobileDatePickerDisplay');
    
    if (mobileDatePickerBtn && datePickerDropdown) {
        // Sync mobile display with desktop
        if (datePickerDisplay) {
            mobileDatePickerDisplay.textContent = datePickerDisplay.textContent;
        }
        
        // Toggle dropdown on mobile button click
        mobileDatePickerBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            datePickerDropdown.classList.toggle('active');
            
            // Position dropdown below mobile button
            const rect = mobileDatePickerBtn.getBoundingClientRect();
            datePickerDropdown.style.position = 'fixed';
            datePickerDropdown.style.left = rect.left + 'px';
            datePickerDropdown.style.top = (rect.bottom + 5) + 'px';
            datePickerDropdown.style.width = rect.width + 'px';
        });
        
        // Update mobile display when date changes
        const originalPopulateMonthDropdown = populateMonthDropdown;
        populateMonthDropdown = function(selected = null) {
            originalPopulateMonthDropdown(selected);
            if (datePickerDisplay && mobileDatePickerDisplay) {
                mobileDatePickerDisplay.textContent = datePickerDisplay.textContent;
            }
        };
    }
...
function addExpenseFromModal(addAnother = false) {
    ...
    // Convert date to "Jan 1" format (like desktop app)
    ...
    if (editId) {
        ...
        if (transaction) {
            ...
            transaction.amount = Math.abs(amount); // Store positive value (desktop app uses positive)
            transaction.category = category;
            transaction.date = formattedDate; // Store as "Jan 1"
            transaction.note = note || '';
            // Keep existing budget_id
        }
        formEl.removeAttribute('data-edit-id');
        showNotification('Expense updated successfully!');
    } else {
        // Create new transaction
        // Get budget_id for current date range
        const budgetId = getBudgetIdForDateRange(currentDateRange);
        
        // Find next row_index for this budget
        const existingExpenses = transactions.filter(t => t.budget_id === budgetId);
        const nextRowIndex = existingExpenses.length > 0 
        ...
        const transaction = {
            id: `${budgetId}_${nextRowIndex}`,
            budget_id: budgetId,
            description: description,
            amount: Math.abs(amount), // Store positive value (desktop app uses positive)
            category: category,
            type: 'expense',
            date: formattedDate, // Store as "Jan 1" format
            note: note || '',
...
// Sync to Firebase only (no localStorage)
function saveTransactions() {
    // Sync all data to Firebase (like desktop app does)
    if (currentUser && currentUser.uid && !currentUser.uid.startsWith('guest_')) {
        syncAllDataToFirebase();
    }
}

// Sync all budgets and expenses to Firebase in desktop app format
function syncAllDataToFirebase() {
    if (!currentUser || !currentUser.uid || currentUser.uid.startsWith('guest_')) {
        return;
    }
    
    // Use in-memory budgets and transactions
    const allTransactions = transactions;
    
    // Convert transactions to desktop app format (expenses array)
    const expenses = allTransactions
        .filter(t => t.type === 'expense' && t.budget_id)
        .map(t => {
            // Extract id and row_index from compound id
            const idParts = t.id.split('_');
            const rowIndex = idParts.length > 1 ? parseInt(idParts[idParts.length - 1]) : 0;
    ...
    // Upload in desktop app format: /users/{uid}/data/{budgets[], expenses[]}
    const dataToSync = {
        budgets: budgets,
        expenses: expenses,
    ...
    database.ref(`users/${currentUser.uid}/data`).set(dataToSync)
        .then(() => {
            console.log(`✓ Synced to Firebase: ${budgets.length} budgets, ${expenses.length} expenses`);
        })
        .catch(err => {
...
function renderTransactionsTable() {
    ...
    // First filter by budget_id if selected (like desktop app does)
    let filtered = transactions;
    if (currentDateRange && currentDateRange !== 'All') {
        const budgetId = getBudgetIdForDateRange(currentDateRange);
        filtered = transactions.filter(t => t.budget_id === budgetId);
        console.log(`Filtered by budget_id "${budgetId}" for date range "${currentDateRange}":`, filtered.length, 'transactions');
    }
...
function updateBudgetOverview() {
    ...
    // Filter transactions by budget_id (like desktop app does)
    let filteredTransactions = transactions;
    if (currentDateRange && currentDateRange !== 'All') {
        const budgetId = getBudgetIdForDateRange(currentDateRange);
        filteredTransactions = transactions.filter(t => t.budget_id === budgetId);
        console.log(`Budget overview filtered by budget_id "${budgetId}":`, filteredTransactions.length, 'transactions');
    }
...
function updateSummaryTable() {
    ...
    // Filter transactions by current date range (like desktop app)
    let filteredTransactions = transactions.filter(t => t.type === 'expense');
    if (currentDateRange && currentDateRange !== 'All') {
        const budgetId = getBudgetIdForDateRange(currentDateRange);
        filteredTransactions = filteredTransactions.filter(t => t.budget_id === budgetId);
        console.log(`Summary filtered by budget_id "${budgetId}":`, filteredTransactions.length, 'expenses');
    }
...
function updateSevenDayChart() {
    ...
    // Filter by budget_id first (like desktop app), then by type
    let expenseTransactions = transactions.filter(t => t.type === 'expense');
    if (currentDateRange && currentDateRange !== 'All') {
        const budgetId = getBudgetIdForDateRange(currentDateRange);
        expenseTransactions = expenseTransactions.filter(t => t.budget_id === budgetId);
        console.log(`Chart filtered by budget_id "${budgetId}":`, expenseTransactions.length, 'expenses');
    }
...
// Get Category Emoji (matching desktop app)
...
function openProfileModal(btnElement) {
    ...
    const modalContent = profileModal.querySelector('.profile-modal-content');
    if (btnElement && modalContent) {
    ...
    const budgetId = getBudgetIdForDateRange();
    ...
    if (currentUser) {
        ...
        if (userEmailEl) userEmailEl.textContent = currentUser.email || 'N/A';
        
        // Show profile photo if available, otherwise show icon
        if (currentUser.photo && profilePhotoEl && profileIconEl) {
            profilePhotoEl.src = currentUser.photo;
            profilePhotoEl.style.display = 'block';
            profileIconEl.style.display = 'none';
        } else if (profilePhotoEl && profileIconEl) {
            profilePhotoEl.style.display = 'none';
            profileIconEl.style.display = 'flex';
        }
        ...
        if (memberSinceEl) {
            ...
            memberSinceEl.textContent = `${monthNames[currentDate.getMonth()]} ${currentDate.getFullYear()}`;
        }
        
        // Update button to show "Sign Out"
        if (googleSignInBtn && googleSignInText) {
            googleSignInText.textContent = 'Sign Out';
        }
    } else {
        // Not signed in
        ...
        if (profilePhotoEl && profileIconEl) {
            profilePhotoEl.style.display = 'none';
            profileIconEl.style.display = 'flex';
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Profile button - show profile modal with real user details
